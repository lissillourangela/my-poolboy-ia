...
const PoolBoy = (() => {
  ...
  const TARGETS = {
    ...
    tac:    { min: 80,  max: 120, warnMin: 60,  warnMax: 150 },
    stabilisant: { min: 30, max: 50, warnMin: 20, warnMax: 100 },
    chloreCombine: { max: 0.5 } // au-delà : chloramines (odeur de chlore, irritation)
  };
  ...
  const Store = {
    ...
    updateMesure(id, changes) {
      const liste = this.getMesures();
      const idx = liste.findIndex((m) => m.id === id);
      if (idx === -1) return;
      liste[idx] = Object.assign({}, liste[idx], changes);
      liste.sort((a, b) => new Date(b.date) - new Date(a.date));
      this.write(STORAGE_KEYS.mesures, liste);
    },
    deleteMesure(id) {
      this.write(STORAGE_KEYS.mesures, this.getMesures().filter((m) => m.id !== id));
    },
    ...
    deleteCarnet(id) {
      this.write(STORAGE_KEYS.carnet, this.getCarnet().filter((e) => e.id !== id));
    },
  ...
  const Diagnostic = {
    ...
    _reglesLocales(m) {
      ...
      if (has(m.chlore) && has(m.chloreTotal)) {
        const combine = parseFloat(m.chloreTotal) - parseFloat(m.chlore);
        if (combine > TARGETS.chloreCombine.max) {
          score -= 15;
          conseils.push({ type: "warn", texte: `Chlore combiné à ${combine.toFixed(1)} mg/L : présence de chloramines (odeur de chlore, irritation). Un traitement choc est recommandé.` });
        }
      }

      if (has(m.tac)) {
        const v = parseFloat(m.tac);
        if (v < TARGETS.tac.warnMin || v > TARGETS.tac.warnMax) {
          score -= 12;
          conseils.push({ type: "warn", texte: `TAC à ${v} mg/L, hors plage de sécurité (60–150). Un TAC déséquilibré rend le pH instable.` });
        } else if (v < TARGETS.tac.min || v > TARGETS.tac.max) {
          score -= 5;
          conseils.push({ type: "warn", texte: `TAC à ${v} mg/L, légèrement hors cible (80–120).` });
        }
      }

      if (has(m.stabilisant)) {
        const v = parseFloat(m.stabilisant);
        if (v > TARGETS.stabilisant.warnMax) {
          score -= 15;
          conseils.push({ type: "alert", texte: `Stabilisant à ${v} mg/L : trop élevé, le chlore perd en efficacité (effet "chlore verrouillé"). Une dilution partielle de l'eau est nécessaire.` });
        } else if (v < TARGETS.stabilisant.warnMin) {
          score -= 8;
          conseils.push({ type: "warn", texte: `Stabilisant à ${v} mg/L, trop bas : le chlore se dégrade vite au soleil.` });
        } else if (v < TARGETS.stabilisant.min || v > TARGETS.stabilisant.max) {
          score -= 4;
          conseils.push({ type: "warn", texte: `Stabilisant à ${v} mg/L, légèrement hors cible (30–50).` });
        }
      }
  ...
  /* =======================================================
     6octies. ÉDITION D'ENTRÉE — modifier la date ou supprimer
     une analyse (historique) ou une action (carnet).
     ======================================================= */
  const EntryEdit = {
    ...
    open(kind, id) {
      const liste = kind === "mesure" ? Store.getMesures() : Store.getCarnet();
      const entry = liste.find((e) => e.id === id);
      if (!entry) return;
      this.current = { kind, id };
      document.getElementById("entry-edit-title").textContent =
        kind === "mesure" ? "Modifier la date de l'analyse" : "Modifier la date de l'action";
      document.getElementById("entry-edit-date").value = this.toLocalInputValue(entry.date);
    ...
    save() {
      const val = document.getElementById("entry-edit-date").value;
      if (!val) { UI.toast("Renseigne une date valide."); return; }
      const iso = new Date(val).toISOString();
      if (this.current.kind === "mesure") Store.updateMesure(this.current.id, { date: iso });
      else Store.updateCarnet(this.current.id, { date: iso });
      this.close();
      UI.renderHistorique();
      UI.renderCarnet();
      UI.renderAccueil();
      UI.toast("Date mise à jour.");
    },

    supprimer() {
      if (!confirm("Supprimer définitivement cette entrée ?")) return;
      if (this.current.kind === "mesure") Store.deleteMesure(this.current.id);
      else Store.deleteCarnet(this.current.id);
      this.close();
      UI.renderHistorique();
      UI.renderCarnet();
      UI.renderAccueil();
  ...
  /* =======================================================
     ...
     /api/diagnostic-ia, qui appelle Claude côté
  ...
  const AIAssistant = {
    async demander() {
      ...
      try {
        const res = await fetch("/api/diagnostic-ia", {
          method: "POST",
  ...
  const TOPICS = [
    ...
    { id: "chlore_combine", titre: "Odeur de chlore / chlore combiné élevé", reponse: "Une forte odeur de chlore ou des yeux qui piquent viennent souvent d'un excès de chloramines (chlore combiné = chlore total moins chlore libre), pas d'un excès de chlore. Solution : traitement choc au chlore non stabilisé, filtration continue jusqu'à ce que le chlore combiné repasse sous 0,5 mg/L." },
    { id: "tac_stabilisant", titre: "TAC et stabilisant déséquilibrés", reponse: "Le TAC (alcalinité, cible 80–120 mg/L) stabilise le pH : trop bas, le pH varie sans arrêt ; trop haut, il devient difficile à corriger. Le stabilisant (acide cyanurique, cible 30–50 mg/L) protège le chlore du soleil, mais au-delà de 100 mg/L il bloque son efficacité (chlore \"verrouillé\") : la seule solution est de diluer l'eau du bassin." }
  ];
  ...
  const UI = {
    ...
    renderAccueil() {
      ...
      const readings = [
        ...
        { label: "Chlore total mg/L", value: m.chloreTotal, cls: "" },
        { label: "TAC mg/L", value: m.tac, cls: this._classFor("tac", m.tac) },
        { label: "Stabilisant mg/L", value: m.stabilisant, cls: this._classFor("stabilisant", m.stabilisant) },
      ...
      el.innerHTML = liste.map(m => {
        ...
        <div class="entry" data-id="${m.id}">
          ...
          <div class="entry-tags">
            ${m.chloreTotal ? `<span class="tag">Cl total ${m.chloreTotal}</span>` : ""}
            ${m.tac ? `<span class="tag">TAC ${m.tac}</span>` : ""}
            ${m.stabilisant ? `<span class="tag">Stab. ${m.stabilisant}</span>` : ""}
  ...
  function init() {
    ...
    document.getElementById("form-analyse").addEventListener("submit", (e) => {
      ...
      const m = {
        ...
        chloreTotal: valOrNull("in-chlore-total"),
        tac: valOrNull("in-tac"),
        stabilisant: valOrNull("in-stabilisant"),
      ...
      if ([m.ph, m.chlore, m.chloreTotal, m.tac, m.stabilisant, m.temperature, m.pression].every(v => v === null)) {
    ...
    // Édition des entrées (date / suppression)
    document.getElementById("historique-list").addEventListener("click", (e) => {
      const entry = e.target.closest(".entry");
      if (entry && entry.dataset.id) EntryEdit.open("mesure", Number(entry.dataset.id));
    });
    document.getElementById("carnet-list").addEventListener("click", (e) => {
      const entry = e.target.closest(".entry");
      if (entry && entry.dataset.id) EntryEdit.open("carnet", Number(entry.dataset.id));
    });
    document.getElementById("btn-entry-edit-save").addEventListener("click", () => EntryEdit.save());
    document.getElementById("btn-entry-edit-delete").addEventListener("click", () => EntryEdit.supprimer());
    document.getElementById("btn-entry-edit-cancel").addEventListener("click", () => EntryEdit.close());
    document.getElementById("entry-edit-backdrop").addEventListener("click", (e) => {
      if (e.target.id === "entry-edit-backdrop") EntryEdit.close();
    });
  ...
  return { Store, Calc, Diagnostic, Charts, Weather, Reminders, Notifications, AIAssistant, PhotoAnalysis, EntryEdit, hooks, TOPICS, GUIDES };
})();
